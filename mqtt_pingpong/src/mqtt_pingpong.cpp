...
_Node::_Node() : Node(GET_NODE_NAME(NODE_NAME)), node_name_(GET_NODE_NAME(NODE_NAME))
{
     ...
     */
    this->pub_my_pong_ = this->create_publisher<std_msgs::msg::String>(GET_TOPIC_NAME(std::string(getEnvOrDefault("PING_PREFIX","")) + std::string("/ros2/pong")), rclcpp::SensorDataQoS().keep_last(1));
    this->sub_ping_ = this->create_subscription<std_msgs::msg::String>(GET_TOPIC_NAME(std::string(getEnvOrDefault("PING_PREFIX","")) + std::string("/ros2/ping")), rclcpp::SensorDataQoS().keep_last(1), std::bind(&_Node::callback_ping, this, std::placeholders::_1));
#ifdef UNIQUE_NODE
